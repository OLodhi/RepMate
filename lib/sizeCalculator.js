...
function calculateSizeFit(sizeData, userMeasurements, garmentType) {
  ...
  for (const [key, ease] of Object.entries(easeConfig)) {
    const garmentValue = sizeData[key];
    // Handle key aliases (topLength -> length, etc.)
    ...
    if (garmentValue != null && userValue != null) {
      ...
      if (key === 'waist' || key === 'hip' || key === 'chest') {
        // Check if this looks like a half measurement (typically < 70 for chest)
        if (garmentValue < 70 && key === 'chest') {
  ...
  const avgDiff = measurementCount > 0 ? totalDiff / measurementCount : 0;

  // Determine overall fit category based on BOTH score AND direction
  // avgDiff > 0 means garment is bigger than body, avgDiff < 0 means garment is smaller
  let fit;
  if (avgScore >= 0.85) {
    fit = 'right';
  } else if (avgScore >= 0.7) {
    // Good score but not perfect - check direction
    fit = avgDiff >= 0 ? 'loose' : 'tight';
  } else if (avgScore >= 0.5) {
    // Moderate score - use direction to determine
    fit = avgDiff >= 0 ? 'oversized' : 'tight';
  } else {
    // Low score - definitely problematic, use direction
    fit = avgDiff >= 0 ? 'too_big' : 'too_small';
  }
