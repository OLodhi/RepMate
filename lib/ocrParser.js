...
function parseOCRSpecificFormat(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (numbers) {
      if (foundMeasurement && !firstSizeFound) {
        // This number is associated with a measurement label (likely S's value)
        measurementValues[foundMeasurement] = parseFloat(numbers[0]);
      }
