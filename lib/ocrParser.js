...
function parseRowBasedTable(text) {
  ...
  // Look for lines that START with a size label (more strict matching)
  const sizeRowPattern = /^[^\w]*(XXS|XS|S|M|L|XL|XXL|XXXL|3XL|4XL|5XL)\b/i;

  for (const line of lines) {
    // Skip header-like lines
    if (/\b(size|shoulder|chest|length|sleeve|waist|hip)\b/i.test(line) &&
        !/^[^\w]*(XXS|XS|S|M|L|XL|XXL|XXXL|3XL|4XL|5XL)\s+\d/i.test(line)) {
      continue;
    }
    ...
    if (sizeMatch) {
      ...
      // Get numbers AFTER the size label
      const afterSize = line.substring(line.search(sizeRowPattern) + sizeMatch[0].length);
      const numbers = afterSize.match(/\b(\d{2,3}(?:\.\d)?)\b/g);
...
function parseByPosition(text) {
  ...
  // Also look for sizes at start of lines with numbers
