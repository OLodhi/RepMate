...
function parseVerticalFormat(text) {
  ...
  // Extract measurement types mentioned in the text
  ...
  // Find which measurements are mentioned and their order
  const foundMeasurements = [];
  const lowerText = text.toLowerCase();

  for (const [keyword, measurement] of Object.entries(measurementKeywords)) {
    if (lowerText.includes(keyword) && !foundMeasurements.includes(measurement)) {
  ...
  if (foundMeasurements.length === 0) {
    foundMeasurements.push('measurement1', 'measurement2', 'measurement3');
  }

  // Extract size labels and their values
  ...
  // Second pass: look for measurement labels with inline values (first size)
  if (rows.length > 0) {
    // We already have size rows, check if there's an implicit first size
      ...
      for (const [keyword, measurement] of Object.entries(measurementKeywords)) {
        if (lowerLine.includes(keyword)) {
          ...
          if (numbers && numbers.length > 0) {
            // Take the first number after the measurement label
    ...
    // If we found first size values, insert as first row
    if (Object.keys(firstSizeValues).length > 0) {
      const firstRow = { size: 'S', ...firstSizeValues };
      // Only add if it doesn't duplicate existing
  ...
  // If still no rows, try alternative extraction
  if (rows.length === 0) {
    // Extract all numbers and try to group them
    const allNumbers = [];
    for (const line of lines) {
      const numbers = line.match(numberPattern);
      if (numbers) {
