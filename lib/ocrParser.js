...
function parseSizeChart(rawText) {
  ...
  // Strategy 0d: Inline format where each line has size + all measurements
  // e.g., "XS: Waist74 Rise32 Thigh66 Pants Length106"
  const inlineSizeResult = parseInlineSizeFormat(translatedText);
  if (inlineSizeResult.rows.length >= 2) {
    results.push(inlineSizeResult);
  }
...
function parseMultipleTables(text) {
  ...
  for (let t = 0; t < sizeRowIndices.length; t++) {
    ...
    // Only check lines before Size row for the FIRST table
    // For subsequent tables, those lines belong to the previous table
    if (t === 0) {
    ...
    if (Object.keys(measurementData).length >= 1) {
      ...
      if (rows.length >= 2) {
        ...
        // Strong bottom indicators (definitely pants)
        const hasStrongBottomIndicators = /pantslength|legopening|inseam|thigh/.test(allMeasurements);

        // Strong top indicators (definitely jacket/shirt)
        const hasStrongTopIndicators = /shoulder|sleeve/.test(allMeasurements);

        // If it has pants length or leg opening, it's definitely bottoms
        // Otherwise, if it has shoulder or sleeve, it's a top
        let garmentType = 'top';
        if (hasStrongBottomIndicators && !hasStrongTopIndicators) {
          garmentType = 'bottom';
        } else if (hasStrongTopIndicators) {
          garmentType = 'top';
        } else if (/hip|thigh/.test(allMeasurements) && !/chest/.test(allMeasurements)) {
          garmentType = 'bottom';
        }
...
/**
 * Parse inline size format where each line contains a size label followed by all measurements
 * Format example:
 *   XS: Waist74 Rise32 Thigh66 Pants Length106
 *   S: Waist78 Rise33 Thigh68 Pants Length106
 *   M: Waist82 Rise34 Thigh70 Pants Length108
 */
function parseInlineSizeFormat(text) {
  const lines = text.split('\n').map(l => l.trim()).filter(l => l);

  // Measurement keywords to look for (order matters - longer first)
  const measurementPatterns = [
    { pattern: /pants\s*length\s*(\d+\.?\d*)/gi, key: 'pantsLength' },
    { pattern: /leg\s*opening\s*(\d+\.?\d*)/gi, key: 'legOpening' },
    { pattern: /waist\s*(\d+\.?\d*)/gi, key: 'waist' },
    { pattern: /chest\s*(\d+\.?\d*)/gi, key: 'chest' },
    { pattern: /shoulder\s*(\d+\.?\d*)/gi, key: 'shoulder' },
    { pattern: /sleeve\s*(\d+\.?\d*)/gi, key: 'sleeve' },
    { pattern: /length\s*(\d+\.?\d*)/gi, key: 'length' },
    { pattern: /hip\s*(\d+\.?\d*)/gi, key: 'hip' },
    { pattern: /thigh\s*(\d+\.?\d*)/gi, key: 'thigh' },
    { pattern: /rise\s*(\d+\.?\d*)/gi, key: 'rise' },
    { pattern: /inseam\s*(\d+\.?\d*)/gi, key: 'inseam' },
  ];

  // Size pattern: XS, S, M, L, XL, etc. followed by : or 码 or just space/numbers
  const sizeLinePattern = /^(XXS|XS|S|M|L|XL|XXL|XXXL|3XL|4XL|5XL)\s*[：:码]?\s*/i;

  const rows = [];
  const headersSet = new Set();

  for (const line of lines) {
    // Check if line starts with a size label
    const sizeMatch = line.match(sizeLinePattern);
    if (!sizeMatch) continue;

    const size = sizeMatch[1].toUpperCase();
    const restOfLine = line.substring(sizeMatch[0].length);

    // Skip if this is just a size label with no measurements
    if (restOfLine.length < 5) continue;

    const row = { size };

    // Extract each measurement from the line
    for (const { pattern, key } of measurementPatterns) {
      pattern.lastIndex = 0; // Reset regex
      const match = pattern.exec(restOfLine);
      if (match) {
        const value = parseFloat(match[1]);
        if (value >= 10 && value <= 250) { // Reasonable measurement range
          row[key] = value;
          headersSet.add(key);
        }
      }
    ...
    // Only add row if we found measurements
    if (Object.keys(row).length > 1) {
      rows.push(row);
    }
  }
