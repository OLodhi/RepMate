...
/**
 * Parse numeric size format where sizes are numbers like 1, 2, 3, 4, 5
 * Format example:
...
function parseNumericSizeFormat(text) {
  ...
  // Look for "Size" followed by numeric sizes (1-9 or 1-5)
  let sizes = [];
  let sizeLineIdx = -1;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lowerLine.includes('size')) {
      // Extract single-digit numbers after "Size"
      const afterSize = line.substring(lowerLine.indexOf('size') + 4);
      const nums = afterSize.match(/\b([1-9])\b/g);
